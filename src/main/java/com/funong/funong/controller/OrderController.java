...
import com.funong.funong.plugin.GetBannerData;
...
public class OrderController {
   ...
   //引用方法
    ...
    @GetMapping("customer/updateGoodOrderSI")
    public Map<String,String> updateGoodOrder(@RequestParam Integer goodOrderId,
                               @RequestParam Integer goodNum,
                               Model model, HttpSession session){
        Goodorder goodorder =new Goodorder();
        goodorder.setGoodorderid(goodOrderId);
        goodorder.setGoodnum(goodNum);
        goodorderDao.updateByPrimaryKeySelective(goodorder);
        Goodorder goodorder1 = goodorderDao.selectByPrimaryKey(goodOrderId);
        checkOrderPrice.checkOrderPrice(goodorder1.getOrderid());
        getBannerData.getBannerData(model,session);
        Good good = goodDao.selectByPrimaryKey(goodorder1.getGoodid());
        Order order = orderDao.selectByPrimaryKey(goodorder1.getOrderid());
        Map<String,String> map = new HashMap<>();

        String orderPrice =  String.valueOf(good.getGoodprice().multiply(BigDecimal.valueOf(goodNum)));
